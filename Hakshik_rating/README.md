...
- **댓글** — 평가의 한줄평이 스레드의 뿌리다. 거기에 답글을 달고, 답글에 또 답글을 달면
  들여쓰기가 한 칸씩 들어간다. 올린 사람과 평가한 사람 둘 다 달 수 있고, 자기가 쓴 것만 지운다.
  **상대가 단 답글은 새로고침 없이 바로 뜬다** (Supabase Realtime).
- **피드** — 가장 최근 기록만 사진을 펼쳐 보여주고, 지난 것들은 같은 내용을 글자로 접어둔다.
  `펼치기` 를 누르면 그 기록의 사진이 펼쳐진다. `최신순 / 별점순` 정렬, 식당은 드롭다운으로 고른다.
...
- **피드가 느리지 않도록** 세 가지를 해뒀다. ① 서명 URL을 만료 시각과 함께 기억해뒀다가
  재사용한다 — 서명할 때마다 토큰이 달라져서 URL이 바뀌면 브라우저가 사진을 매번 새로
  받는다. ② 마지막으로 받은 목록을 먼저 그려놓고 뒤에서 갱신한다. ③ 화면 코드는 필요할 때
  불러온다 (EXIF 라이브러리 26KB는 올리기 화면을 열 때만 받는다).
- **카드는 사진 비율을 그대로 따라간다.** 비율을 고정해두면 가로 사진이 좌우로 크게
  잘려나간다. 피드에 사진이 한 장만 펼쳐지므로 높이가 달라져도 보기 나쁘지 않다.
  크기는 올릴 때 `photo_w`/`photo_h` 에 같이 저장한다 — 사진을 받아봐야 알 수 있으면
  로딩 중에 카드 높이가 튄다. 크기가 없는 예전 기록은 4:5 로 본다.
...
js/lib/photo.js       업로드용 리사이즈
js/lib/exif.js        EXIF 추출 (무거워서 올리기 화면에서만 불린다)
js/lib/recompress.js  이미 올라간 사진 다시 압축
js/lib/realtime.js    댓글 실시간 구독
